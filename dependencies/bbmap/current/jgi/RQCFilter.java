...
//		assert(false) : rnaArtifactFlag+"\n"+primaryArgList+"\n"+libType+"\n"+outDir;
		...
		doSpikein=(mtstFlag || !spikeinRefs.isEmpty());
...
//				assert(false) : inPrefix+", "+outPrefix+", "+in1+", "+out1;
				...
				String ref=taxFilter(commonMicrobesRef, "taxa.fa.gz", false);
...
//				System.err.println("in1z="+in1z+"\nout1z="+out1z+"\ninPrefix="+inPrefix+"\noutPrefix="+outPrefix);
				
				{
					...
					if(!f.exists() || f.length()<200){
						System.err.println("Can't find chloroplast/mito/ribo for taxa; using full dataset.");
						ref=ref0;
					}
				}
...
//			argList.add("loglog"); //Cardinality
		...
		{//Run Seal
			...
			try {
				seal.process();
				System.err.println(format("Spikein Sequence Removed:", seal.readsIn, seal.readsMatched, seal.basesIn, seal.basesMatched));
...
//			for(String s : primaryArgList){argList.add(s);}
	 ...
	 */
	private String taxFilter(String in, String out, boolean include){
		if(taxList==null){
...
//			assert(false) : scafstats+", "+microbeStatsFile;
	...
	private boolean runSketch_inner(String in1, String fname, String db, boolean append){
		SketchObject.reset();
		ArrayList<String> argList=new ArrayList<String>();
		argList.add("in="+in1);
		...
		try {
			ReadWrite.writeString("DB:\t"+db, fname, append);
			
			SendSketch.main(argList.toArray(new String[0]));
			return true;
		} catch (Throwable e) {
